...
<-fs-> {
  ...
  <-float3-> specular = 0.6 * pow(max(dot(normalize(-ViewPosition), reflect(light_direction, normal)), 0.0), 16) * light_color;

  <-float3-> result = ambient + diffuse + specular;

  <-fs_color-> = <-float4->(result, 1.0) * <-float4->(1.0, 0.0, 0.0, 1.0);
}
