...
typedef function<void(EventContext* context)> EventCallback;
...
class UIEventDispatcher : public Ref
{
    FUI_RTTI_DERIVED(UIEventDispatcher, Ref)
public:
    UIEventDispatcher();
...
private:
    ...
    vector<EventCallbackItem> _callbacks;
    int _dispatching;
};
