...
TreeView::~TreeView()
{
    FUI_SAFE_RELEASE(_rootNode);
    FUI_SAFE_RELEASE(_list);
}
...
bool TreeView::init(GList * list)
{
    _list = list;
    _list->retain();
    _list->addEventListener(UIEventType::ClickItem, FUI_CALLBACK_1(TreeView::onClickItem, this));
    _list->addEventListener(UIEventType::RightClickItem, FUI_CALLBACK_1(TreeView::onClickItem, this));
...
void TreeView::getSelection(vector<TreeNode*>& result) const
{
    vector<int> ids;
...
void TreeView::createCell(TreeNode * node)
{
    ...
    f_assert2(obj, "Unable to create tree cell");
    node->setCell(obj);
    ...
    if (expandButton != nullptr)
    {
        if (node->isFolder())
        {
            ...
            expandButton->addClickListener(FUI_CALLBACK_1(TreeView::onClickExpandButton, this));
...
{
    if (node != _rootNode && treeNodeWillExpand)
