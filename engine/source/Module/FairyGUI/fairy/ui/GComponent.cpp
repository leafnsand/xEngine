...
{
    m_displayObject = new Container("GComponent",this);
}
...
GComponent::~GComponent()
{
    ...
    FUI_SAFE_RELEASE(_maskOwner);
    FUI_SAFE_RELEASE(_scrollPane);
    FUI_SAFE_DELETE(_hitArea);
...
GObject* GComponent::addChild(GObject* child, Container* realFatherContainer)
{
    addChildAt(child, (int)_children.size(),realFatherContainer);
    return child;
}

GObject* GComponent::addChildAt(GObject* child, int index, Container* realFatherContainer)
{
    f_assert2(child != nullptr, "Argument must be non-nil");
    ...
    else
    {
        ...
        if (realFatherContainer == nullptr)
        {
            ((Container*)this->m_displayObject)->AddChild(child->m_displayObject);
        }
        else
        {
            realFatherContainer->AddChild(child->m_displayObject);
        }
...
void GComponent::removeChildAt(int index)
{
    ...
    ((Container*)m_displayObject)->RemoveChild(child->m_displayObject);
...
GObject* GComponent::getChild(const AZStd::string& name) const
{
...
GObject* GComponent::getChildInGroup(const GGroup* group, const AZStd::string& name) const
{
    f_assert2(group != nullptr, "Argument must be non-nil");
...
GObject* GComponent::getChildById(const AZStd::string& id) const
{
    for (const auto& child : _children)
    {
...
void GComponent::setChildIndex(GObject* child, int index)
{
    ...
    f_assert2(oldIndex != -1, "Not a child of this container");
...
{
    f_assert2(child1 != nullptr, "Argument1 must be non-nil");
    f_assert2(child2 != nullptr, "Argument2 must be non-nil");

    int index1 = (int)_children.getIndex(child1);
    int index2 = (int)_children.getIndex(child2);

    f_assert2(index1 != -1, "Not a child of this container");
    f_assert2(index2 != -1, "Not a child of this container");
...
bool GComponent::isChildInView(GObject * child)
{
    ...
    else if (this->isClippingEnabled())
    {
...
GController* GComponent::getController(const AZStd::string& name) const
{
...
{
    f_assert2(c != nullptr, "Argument must be non-nil");
...
void GComponent::removeController(GController* c)
{
    ...
    f_assert2(index != -1, "controller not exists");
...
Transition * GComponent::getTransition(const AZStd::string & name) const
{
...
void GComponent::adjustRadioGroupDepth(GObject* obj, GController* c)
{
    ...
    for (i = 0; i < cnt; i++)
    {
        ...
        else if (child->SafeCast<GButton>()
...
GObject* GComponent::getMask() const
{
    if(this->_scrollPane)
    {
        return this->_scrollPane->_maskContainer;
    }
    if(_maskOwner)
    {
        return _maskOwner;
...
void GComponent::setMask(GObject * value, bool inverted)
{
    if (_maskOwner)
    {
...
void GComponent::childStateChanged(GObject* child)
{
    ...
    if (child->SafeCast<GGroup>() != nullptr)
    {
        for (int i = 0; i < cnt; ++i)
        {
            GObject* g = _children.at(i);
    ...
    if (child == nullptr || child == _maskOwner)
        return;
...
//            if (child->_displayObject != nullptr && child != _maskOwner && child->internalVisible())
            if (child != nullptr && child != _maskOwner && child->internalVisible())
            {
...
Vec2 GComponent::getSnappingPosition(const Vec2 & pt)
{
    int cnt = (int)_children.size();
    if (cnt == 0)
        return pt;
...
GObject* GComponent::hitTest(const Vec2 &worldPoint)
{
//    if (_touchDisabled || !_touchable || !_displayObject->isVisible() || !_displayObject->getParent())
    if (_touchDisabled || !_touchable || !isVisible() )//|| !getParent())
        return nullptr;

    GObject * target = nullptr;
    if (_maskOwner)
    {
        if (_maskOwner->hitTest(worldPoint) != nullptr)
        {
...
//            if (!((FUIContainer*)_displayObject)->isInverted())
    ...
    else
    {
        if (this->isClippingEnabled())
        {
            ...
            const Rect& clipRect = this->getClippingRegion();
...
//        if (!(*it)->_displayObject || *it == _maskOwner)
    ...
    if (_opaque)
    {
        ...
        {
            rect.size = _size;
            flag = rect.containsPoint(this->convertToNodeSpace(worldPoint)) ? 1 : 2;
        }
...
//        _container->setPosition2(_margin.left, _margin.top);
    ...
    if (_hitArea)
    {
        PixelHitTest* test = _hitArea->SafeCast<PixelHitTest>();
...
void GComponent::constructFromResource(vector<GObject*>* objectPool, int poolIndex)
{
...
//        setMask(getChildById(p)->displayObject(), inverted);
        setMask(getChildById(p), inverted);
    }
...
Transition* GComponent::addNewTransition(const Transition& transition, const GObject& object)
{
    auto exist = this->getTransition(transition.name);
    if (exist)
        return exist;

    Transition* trans = new Transition(this, (int)_transitions.size());
    trans->clone(transition, object);
    _transitions.pushBack(trans);

    return trans;
}

void GComponent::RemoveTransition(const char* name)
{
    Transition* trans = nullptr;
    for (auto& c : _transitions)
    {
        if (c->name.compare(name) == 0)
        {
            trans = c;
    ...
    if (trans)
    {
        ...
        _transitions.eraseObject(trans);
        trans->release();
    }
