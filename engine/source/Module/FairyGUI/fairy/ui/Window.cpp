...
Window::Window() :
    ...
    _loading(false)
{
    _bringToFontOnClick = UIConfig::getInstance()->bringWindowToFrontOnClick;
}

Window::~Window()
{
    FUI_SAFE_RELEASE(_contentPane);
    FUI_SAFE_RELEASE(_frame);
    FUI_SAFE_RELEASE(_closeButton);
    FUI_SAFE_RELEASE(_dragArea);
...
void Window::handleInit()
{
    ...
    addEventListener(UIEventType::TouchBegin, FUI_CALLBACK_1(Window::onTouchBegin, this));
}
...
void Window::setContentPane(GComponent* value)
{
    if (_contentPane != value)
    {
        ...
        if (_contentPane != nullptr)
        {
            ...
            auto obj = _contentPane->getChild("frame");
            if(obj)
            {
                _frame = obj->SafeCast<GComponent>();
                if (_frame != nullptr)
                {
                    _frame->retain();
...
void Window::setCloseButton(GObject * value)
{
    ...
    {
        _closeButton->retain();
        _closeButton->addClickListener(FUI_CALLBACK_1(Window::closeEventHandler, this), EventTag(this));
    }
}
...
void Window::setDragArea(GObject * value)
{
    if (_dragArea != value)
    {
        ...
        {
            _dragArea->retain();
            if (_dragArea->SafeCast<GGraph>() && ((GGraph*)_dragArea)->isEmpty())
            ...
            _dragArea->addEventListener(UIEventType::DragStart, FUI_CALLBACK_1(Window::onDragStart, this), EventTag(this));
        }
    }
}
...
void Window::showModalWait(int requestingCmd)
{
    ...
    if (!UIConfig::getInstance()->windowModalWaiting.empty())
    {
        if (_modalWaitPane == nullptr)
        {
            _modalWaitPane = UIPackage::createObjectFromURL(UIConfig::getInstance()->windowModalWaiting);
...
void Window::initWindow()
{
    ...
    if (!_uiSources.empty())
    {
        ...
        for (int i = 0; i < cnt; i++)
        {
            ...
            if (!lib->isLoaded())
            {
                lib->load(FUI_CALLBACK_0(Window::onUILoadComplete, this));
                _loading = true;
            }
        }
