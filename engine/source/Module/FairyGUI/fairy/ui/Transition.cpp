...
TransitionItem::TransitionItem() :
    ...
    duration(0),
    easeType(TweenType::Quad_EaseOut),
    repeat(0),
    yoyo(false),
    tween(false),
    hook(nullptr),
...
void Transition::stopItem(TransitionItem * item, bool setToComplete)
{
    ...
    if (item->actionPtr != nullptr)
    {
        switch (item->type)
        {
        case TransitionActionType::XY:
        case TransitionActionType::Size:
            ((Action<Vec2>*) item->actionPtr)->Kill();
            break;
        ...
        case TransitionActionType::Rotation:
            ((Action<float>*) item->actionPtr)->Kill();
            break;
        case TransitionActionType::Color:
            ((Action<Vec4>*) item->actionPtr)->Kill();
            break;
        case TransitionActionType::ColorFilter:
            ((Action<Vec4>*) item->actionPtr)->Kill();
            break;
        }
        item->actionPtr = nullptr;
    }
}

void Transition::setValue(const AZStd::string & label, const ValueVector& values)
{
    for (auto &item : _items)
    {
...
void Transition::setHook(const AZStd::string & label, TransitionHook callback)
{
    for (auto &item : _items)
    {
        if (item->label == label)
...
void Transition::setTarget(const AZStd::string & label, GObject * newTarget)
{
    for (auto &item : _items)
    {
        if (item->label == label)
...
void Transition::setDuration(const AZStd::string & label, float value)
{
    for (auto &item : _items)
    {
...
void Transition::updateFromRelations(const AZStd::string & targetId, float dx, float dy)
{
    int cnt = (int)_items.size();
    if (cnt == 0)
        return;
...
//    ActionInterval* mainAction = nullptr;
    ...
    case TransitionActionType::Size:
    {
        ...
        action->SetTweenData(Vec2(startValue.f1, startValue.f2), Vec2(endValue.f1, endValue.f2), delay, item->duration, item->easeType, [this, item](const Vec2& value)
        {
        ...
        if (item->repeat != 0)
        {
            action->SetLoopType(item->yoyo ? LoopType::Yoyo : LoopType::Loop, item->repeat);
        }

        action->Run();
        item->actionPtr = action;
        break;
    }
    ...
    case TransitionActionType::Rotation:
    {
        ...
        Action<float>* action = Action<float>::create();
        action->SetTweenData(startValue.f1, endValue.f1, delay, item->duration, item->easeType, [this, item](const float& value)
        {
            item->value.f1 = value;
            applyValue(item, item->value);
        }, [this, item]()
    ...
    case TransitionActionType::Color:
    {
        ...
        Action<Vec4>* action = Action<Vec4>::create();
        action->SetTweenData(Vec4(item->value.c.r, item->value.c.g, item->value.c.b, item->value.c.a), Vec4(endValue.c.r, endValue.c.g, endValue.c.b, endValue.c.a), delay, item->duration, item->easeType, [this, item](const Vec4& value)
        {
    ...
    case TransitionActionType::ColorFilter:
    {
        ...
        Action<Vec4>* action = Action<Vec4>::create();
        action->SetTweenData(Vec4(startValue.f1, startValue.f2, startValue.f3, startValue.f4), Vec4(endValue.f1, endValue.f2, endValue.f3, endValue.f4), delay, item->duration, item->easeType, [this, item](const Vec4& value)
        {
            item->value.f1 = value.x;
            item->value.f2 = value.y;
...
void Transition::applyValue(TransitionItem * item, TransitionValue & value)
{
    ...
    case TransitionActionType::Animation:
    {
        IAnimationGear* ag = item->target->GetIAnimationGear();
        if (ag)
        {
            if (!value.b1)
...
//        Director::getInstance()->getScheduler()->schedule(FUI_CALLBACK_1(Transition::shakeItem, this, item), item, 0, false, "-");
        _totalTasks++;
        item->completed = false;
        break;
...
#define rand_0_1() (float)(rand() % 100) / (float)100
...
void Transition::clone(const Transition& transition, const GObject& object)
{
    ...
    this->_options = transition._options;
    this->_autoPlay = transition._autoPlay;
    this->autoPlayRepeat = transition.autoPlayRepeat;
    this->autoPlayDelay = transition.autoPlayDelay;
    this->_maxTime = transition._maxTime;
    for (auto& it : transition._items)
    {
        TransitionItem* item = new TransitionItem();
        _items.push_back(item);
        ...
        item->easeType = it->easeType;
        item->repeat = it->repeat;
        item->yoyo = it->yoyo;
        item->label2 = it->label2;
        item->startValue = it->startValue;
        item->value = it->value;
        item->endValue = it->endValue;
        auto oldTarget = transition._owner->getChildById(it->targetId);
        switch (it->type)
        {
        case TransitionActionType::XY:
            if (it->startValue.b1)
            {
                item->startValue.f1 = it->startValue.f1 - oldTarget->getX() + object.getX();
            }
            if (it->startValue.b2)
            {
                item->startValue.f2 = it->startValue.f2 - oldTarget->getY() + object.getY();
            }
            if (it->endValue.b1)
            {
                item->endValue.f1 = it->endValue.f1 - oldTarget->getX() + object.getX();
            }
            if (it->endValue.b2)
            {
                item->endValue.f2 = it->endValue.f2 - oldTarget->getY() + object.getY();
            }
            break;
        }
    }
