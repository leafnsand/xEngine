...
namespace AZ
{
    ...
    public:
        ...
        ///Stores the matrix into to an array of 16 floats. The floats need only be 4 byte aligned, 16 byte alignment is NOT required.
        AZ_MATH_FORCE_INLINE void StoreToColumnMajorFloat16Ex(float* values) const
        {
            GetColumn(0).StoreToFloat3(values);         *(values + 3) = 0;
            GetColumn(1).StoreToFloat3(values + 4);     *(values + 7) = 0;
            GetColumn(2).StoreToFloat3(values + 8);     *(values + 11) = 0;
            GetColumn(3).StoreToFloat3(values + 12);    *(values + 15) = 1;
        }
