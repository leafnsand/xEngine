...
namespace AZ
{
    namespace IO
    {
        ...
        bool CreateTempFileName(const char* file, AZStd::string& tempFile)
        {
            ...
            if (!AZ::StringFunc::Path::GetFullPath(file, fullPath))
            {
                AZ_Warning("AZ::IO::CreateTempFileName", false, " Filepath needs to be an absolute path: '%s'", file);
                return false;
            }

            if (!AZ::StringFunc::Path::GetFullFileName(file, fileName))
            {
                AZ_Warning("AZ::IO::CreateTempFileName", false, " Filepath needs to be an absolute path: '%s'", file);
                return false;
            }
            ...
            {
                AZ::StringFunc::Path::ConstructFull(fullPath.c_str(), AZStd::string::format("$tmp%d_%s", rand(), fileName.c_str()).c_str(), tempFile, true);
