...
namespace AZ
{
    ...
    public:
        ...
        */
        ...
        /**
        * Returns the total number of handlers that are connected to the specific id of EBus.
        * @return The total number of handlers that are connected to the specific id of EBus.
        */
        static size_t GetTotalNumOfEventHandlersOfBusId(const BusIdType& id);
...
#pragma warning(pop)
    ...
    //=========================================================================
    // GetTotalNumOfEventHandlersOfBusId
    //=========================================================================
    template<class Interface, class Traits>
    size_t  EBus<Interface, Traits>::GetTotalNumOfEventHandlersOfBusId(const BusIdType& id)
    {
        size_t size = 0;
        Context* context = GetContext();
        if (context)
        {
            context->m_mutex.lock();
            typename BusesContainer::iterator node = context->m_buses.find(id);
            if (node != context->m_buses.end())
            {
                size = node->size();
            }
            context->m_mutex.unlock();
