...
static int
__pack_packed_varint(struct _pattern_field *pf , struct pbc_slice *slice, pbc_array array) {
...
static int 
__pack_packed(struct _pattern_field *pf , struct pbc_slice *s, pbc_array array) {
	int n = pbc_array_size(array);
	if (n == 0)
		return 0;
	...
	case PTYPE_SINT32:
	case PTYPE_SINT64:
		len = __pack_packed_varint(pf, s, array);
		if (len < 0)
			return len;
		break;
	}
...
pbc_pattern_pack(struct pbc_pattern *pat, void *input, struct pbc_slice * s)
{
	...
	for (i=0;i<pat->count;i++) {
		...
		case LABEL_REPEATED:
			...
			break;
		case LABEL_PACKED:
			len = __pack_packed(pf, &slice , (struct _pbc_array *)in);
			break;
		}
		if (len < 0) {
			return len;
