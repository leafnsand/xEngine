...
namespace AZ
{
    ...
    void UtilOprate::Reflect(AZ::ReflectContext* context)
    {
        if (auto behaviorContext = azrtti_cast<AZ::BehaviorContext*>(context)) 
        {
            behaviorContext->Class<UtilOprate>("UtilOprate")
                ->Method("SaveObjectToFile", &UtilOprate::SaveObjectToFile)
                ->Method("SaveAssetToFile", &UtilOprate::SaveAssetToFile);
        }
    }

    void UtilOprate::SaveObjectToFile(const AZStd::string& filePath, AZ::Entity* classPtr)
    {
        Utils::SaveObjectToFile<AZ::Entity>(filePath, DataStream::ST_XML, classPtr);
    }
    
    void UtilOprate::SaveAssetToFile(const AZStd::string& filePath, AZ::Data::AssetData* classPtr, const Uuid& classId)
    {
        Utils::SaveObjectToFile(filePath, DataStream::ST_XML, classPtr, classId);
    }
} // namespace AZ
