...
namespace AZ
{
    ...
    {
    public:
        AZ_RTTI(UtilOprate, "{561C8C7A-6A3C-4A59-AF1E-E3FECE6FDF9A}");
        AZ_CLASS_ALLOCATOR(UtilOprate, AZ::SystemAllocator, 0);

        static void Reflect(AZ::ReflectContext* context);

        static void SaveObjectToFile(const AZStd::string& filePath, AZ::Entity* classPtr);

        static void SaveAssetToFile(const AZStd::string& filePath, AZ::Data::AssetData* classPtr, const Uuid& classId);
    };
} // namespace AzCore
