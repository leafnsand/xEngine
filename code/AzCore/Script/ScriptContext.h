...
namespace AZ
{
    ...
    {       
        ...
        friend class ScriptContextDebug;
    ...
    typedef Uuid ScriptTypeId;
    typedef size_t ScriptShortTypeId;
    ...
    struct ScriptValueGeneric<T, true>
    {
        ...
        {
            Internal::azlua_tointeger(l, static_cast<int>(value)); // treat enums as ints
        }
        static inline T  StackRead(lua_State* l, int stackIndex)
        {
            return static_cast<T>(Internal::azlua_tointeger(l, stackIndex));
        }
    };
    ...
    {
        static const bool isNativeValueType = true;  // We use native type for internal representation
        static void StackPush(lua_State* l, T* pointer, const AZStd::true_type& /* Internal::HasAzTypeInfo<T>::value() */)
        {
            Internal::LuaScriptValueStackPush(l, pointer, &AzTypeInfo<T>::Uuid());
        }
        static T*   StackRead(lua_State* l, int stackIndex, const AZStd::true_type& /* Internal::HasAzTypeInfo<T>::value() */)
        {
            return reinterpret_cast<T*>(Internal::LuaScriptValueStackRead(l, stackIndex, &AzTypeInfo<T>::Uuid()));
        }
        static void StackPush(lua_State* l, T* pointer, const AZStd::false_type& /* Internal::HasAzTypeInfo<T>::value() */)
        {
            Internal::LuaScriptValueStackPush(l, pointer, nullptr);
        }
        static T*   StackRead(lua_State* l, int stackIndex, const AZStd::false_type& /* Internal::HasAzTypeInfo<T>::value() */)
        {
            return reinterpret_cast<T*>(Internal::LuaScriptValueStackRead(l, stackIndex, nullptr));
        }
        static void StackPush(lua_State* l, T* pointer)
        {
            StackPush(l, pointer, typename Internal::HasAZTypeInfo<T>::type());
        }
        static T*   StackRead(lua_State* l, int stackIndex)
        {
            return StackRead(l, stackIndex, typename Internal::HasAZTypeInfo<T>::type());
        }
    };
    ...
    template<>
    struct ScriptValue<const VectorFloat>
    {
        static const bool isNativeValueType = true;  ///< We represent the class as a native number
        static void StackPush(lua_State* l, float value);
        static float StackRead(lua_State* l, int stackIndex);
    };
    template<>
    struct ScriptValue<const VectorFloat&>
        : public ScriptValue<const VectorFloat>{};
    template<>
    struct ScriptValue<VectorFloat>
        : public ScriptValue<const VectorFloat>{};
    ...
    bool ScriptDataContext::ReadTableElement(int tableIndex, const char* name, T& value)
    {
        ...
        {
            bool result = false;
            Internal::azlua_getfield(m_nativeContext, m_startVariableIndex + tableIndex, name);
            if (!Internal::azlua_isnil(m_nativeContext, -1))
            {
                // TODO make a type check
                value = ScriptValue<T>::StackRead(m_nativeContext, -1);
                result = true;
            }
            Internal::azlua_pop(m_nativeContext, 1);
            return result;
        }
        else
        {
    ...
    void ScriptDataContext::AddReplaceTableElement(int tableIndex, const char* name, const T& value)
    {
        if (m_mode == MD_INSPECT)
        {
            Internal::azlua_pushstring(m_nativeContext, name);
            ScriptValue<T>::StackPush(m_nativeContext, value);
            Internal::azlua_rawset(m_nativeContext, m_startVariableIndex + tableIndex);
        }
        else
        {
    ...
    {
        Internal::LuaClassToStack(l, (void*)&value, AzTypeInfo<T>::Uuid());
    }

    template<class T, bool isEnum>
    inline T ScriptValueGeneric<T, isEnum>::StackRead(lua_State* l, int stackIndex)
    {
        return *reinterpret_cast<T*>(Internal::LuaClassFromStack(l, stackIndex, AzTypeInfo<T>::Uuid()));
    }
    ...
    */
    ...
    public:
        ...
        ScriptContext(IAllocatorAllocate* allocator = nullptr, lua_State* nativeContext = nullptr);
        ~ScriptContext();
         ...
         */
        bool LoadFromStream(IO::GenericStream* stream, const char* debugName, lua_State* thread = nullptr);
    ...
    bool ScriptContext::ReadGlobal(const char* name, T& value)
    {
        ...
        Internal::azlua_getglobal(nativeContext, name);
        if (!Internal::azlua_isnil(nativeContext, -1))
        {
            value = ScriptValue<T>::StackRead(nativeContext, -1);
            result = true;
        }
        Internal::azlua_pop(nativeContext, 1);
        return result;
    }
    ...
    bool ScriptContext::ReadGlobal(int cacheIndex, T& value)
    {
        bool result = false;
        lua_State* nativeContext = NativeContext();
        Internal::LuaRegistryGet(nativeContext, cacheIndex);
        if (!Internal::azlua_isnil(nativeContext, -1))
        {
            value = ScriptValue<T>::StackRead(nativeContext, -1);
            result = true;
    ...
    inline void ScriptContext::AddReplaceGlobal(const char* name, const T& value)
    {
        ...
        Internal::azlua_setglobal(nativeContext, name);
    }
    ...
    inline void ScriptContext::AddReplace(int cacheIndex, const T& value)
    {
        lua_State* nativeContext = NativeContext();
        ScriptValue<T>::StackPush(nativeContext, value);
        Internal::LuaRegistrySet(nativeContext, cacheIndex);
    }
    //////////////////////////////////////////////////////////////////////////
    //////////////////////////////////////////////////////////////////////////
    //////////////////////////////////////////////////////////////////////////
