...
namespace AZ
{
    ...
    bool SliceComponent::Instantiate()
    {
        ...
        if (m_slicesAreInstantiated)
        {
            if (m_myAsset)
            {
                AZStd::string sliceAssetPath = Data::AssetManager::Instance().GetAssetPathById(m_myAsset->GetId());
    ...
    void SliceComponent::GenerateNewEntityIds(EntityIdToEntityIdMap* previousToNewIdMap)
    {
        ...
        AZ::IdUtils::Remapper<EntityId>::GenerateNewIdsAndFixRefs(&entityContainer, *previousToNewIdMap, m_serializeContext);
    ...
    bool SliceComponent::GetEntityRestoreInfo(const EntityId entityId, EntityRestoreInfo& restoreInfo)
    {
        ...
        if (entityInfoIter != entityInfo.end())
        {
            ...
            if (reference)
            {
                ...
                if (!ancestors.empty())
                {
                    restoreInfo = EntityRestoreInfo(reference->GetSliceAsset(), instance->GetId(), ancestors.front().m_entity->GetId());
                    return true;
                }
                else
                {
    ...
    void SliceComponent::Reflect(ReflectContext* reflection)
    {
        ...
        if (serializeContext)
        {
            ...
            serializeContext->Class<SliceInstance>()->
                ...
                Field("EntityIdMap", &SliceInstance::m_baseToNewEntityIdMap); // added at v3
            ...
            serializeContext->Class<EntityRestoreInfo>()->
                ...
                Field("AncestorId", &EntityRestoreInfo::m_ancestorId); // added at v1
        }

        BehaviorContext* behaviorContext = azrtti_cast<BehaviorContext*>(reflection);
        if (behaviorContext)
        {
            behaviorContext->Class<SliceComponent>()
                ->Attribute(AZ::Script::Attributes::ExcludeFrom, AZ::Script::Attributes::ExcludeFlags::Preview)
                ->Constructor()
                ->Method("AddEntity", &SliceComponent::AddEntity);
        }
    }
} // namespace AZ
