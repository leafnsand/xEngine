        ...
        pthread_t create_thread(const thread_desc* desc, thread_info* ti)
        {
            ...
            if (desc)
            {
                ...
                if (desc->m_cpuId >= 0) // TODO: add a proper mask to support more than one core that can run the thread!!!
                {
                    ...
                    AZ_Warning("System", true, "pthread_attr_setaffinity_np not supported on EMSCRIPTEN!\n");
#elif !defined(AZ_PLATFORM_APPLE) && !defined(AZ_PLATFORM_ANDROID)
                    cpu_set_t cpuset;
                    CPU_ZERO(&cpuset);
            ...
            AZ_Warning("System", true, "pthread_setname_np not supported on EMSCRIPTEN!\n");
#else
