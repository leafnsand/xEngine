...
{
    FUI_RTTI_DERIVED(GList, GComponent)
public:
    typedef function<void(int, GObject*)> ListItemRenderer;
    typedef function<AZStd::string(int)> ListItemProvider;
    ...
    const AZStd::string& getDefaultItem() const { return _defaultItem; }
    void setDefaultItem(const AZStd::string& value) { _defaultItem = value; }
    ...
    GObject* getFromPool() { return getFromPool(_defaultItem); }
    GObject* getFromPool(const AZStd::string& url);
    void returnToPool(GObject *obj);
    GObject* addItemFromPool() { return addItemFromPool(_defaultItem); }
    GObject* addItemFromPool(const AZStd::string& url);

    GObject* addChildAt(GObject* child, int index,Container* container = nullptr) override;
    ...
    void getSelection(vector<int>& result);
    ...
    virtual Vec2 getSnappingPosition(const Vec2& pt) override;

    void setItemRenderer(ListItemRenderer func) { itemRenderer = func; }
