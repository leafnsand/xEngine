...
void GList::setAlign(TextHAlignment value)
{
    if (_align != value)
    {
        _align = value;
...
void GList::setVerticalAlign(TextVAlignment value)
{
...
GObject * GList::getFromPool(const AZStd::string& url)
{
    GObject* ret;
    if (url.length() == 0)
...
GObject * GList::addItemFromPool(const AZStd::string & url)
{
...
GObject * GList::addChildAt(GObject * child, int index, Container* container)
{
    GComponent::addChildAt(child, index, container);
    if (child->SafeCast<GButton>())
    {
    ...
    child->addEventListener(UIEventType::TouchBegin, FUI_CALLBACK_1(GList::onItemTouchBegin, this), EventTag(this));
    child->addClickListener(FUI_CALLBACK_1(GList::onClickItem, this), EventTag(this));
    child->addEventListener(UIEventType::RightClick, FUI_CALLBACK_1(GList::onClickItem, this), EventTag(this));
...
{
    f_assertmsg(index < _children.size(), "index is over, index = %d, size = %d", index, _children.size());
...
int GList::getSelectedIndex() const
{
    if (_virtual)
    {
        ...
        for (int i = 0; i < cnt; i++)
        {
            ...
            if ((ii.obj->SafeCast<GButton>() != nullptr && ((GButton*)ii.obj)->isSelected()) || (ii.obj == nullptr && ii.selected))
            {
                if (_loop)
                    return i % _numItems;
                else
...
void GList::getSelection(vector<int>& result)
{
    result.clear();
    if (_virtual)
    {
        ...
        for (int i = 0; i < cnt; i++)
        {
            ...
            if ((ii.obj->SafeCast<GButton>() != nullptr && ((GButton*)ii.obj)->isSelected())
...
void GList::selectAll()
{
    ...
    if (_virtual)
    {
        ...
        {
            ItemInfo& ii = _virtualItems[i];
            if (ii.obj->SafeCast<GButton>() && !((GButton*)ii.obj)->isSelected())
            {
...
void GList::onItemTouchBegin(EventContext * context)
{
    ...
    if (UIConfig::getInstance()->defaultScrollTouchEffect
...
{
    if (!(item->SafeCast<GButton>()) || _selectionMode == ListSelectionMode::NONE)
        return;
...
void GList::scrollToView(int index, bool ani, bool setFirst)
{
    if (_virtual)
    {
        ...
        f_assert2(index >= 0 && index < (int)_virtualItems.size(), "Invalid child index");
...
void GList::setVirtual(bool loop)
{
    if (!_virtual)
    {
        f_assert2(_scrollPane != nullptr, "FairyGUI: Virtual list must be scrollable!");
        if (_scrollPane == nullptr) return;

        if (loop)
        {
            f_assert2(_layout != ListLayoutType::FLOW_HORIZONTAL && _layout != ListLayoutType::FLOW_VERTICAL,
        ...
        {
            GObject *obj = getFromPool();
            f_assert2(obj != nullptr, "FairyGUI: Virtual List must have a default list item resource.");
        ...
        addEventListener(UIEventType::Scroll, FUI_CALLBACK_1(GList::onScroll, this));
        setVirtualListChangedFlag(true);
    }
}
...
void GList::setNumItems(int value)
{
    if (_virtual)
    {
        f_assert2(itemRenderer, "FairyGUI: Set itemRenderer first!");
...
{
    f_assert2(_virtual, "FairyGUI: not virtual list");

    setVirtualListChangedFlag(false);
}

Vec2 GList::getSnappingPosition(const Vec2 & pt)
{
    if (_virtual)
    {
        Vec2 ret = pt;
...
int GList::getIndexOnPos1(float & pos, bool forceUpdate)
{
    ...
    {
        float pos2 = getChildAt(0)->getY()-getMargin().top;
...
int GList::getIndexOnPos2(float & pos, bool forceUpdate)
{
    ...
    if (numChildren() > 0 && !forceUpdate)
    {
        float pos2 = getChildAt(0)->getX() - getMargin().left;
...
void GList::handleScroll1(bool forceUpdate)
{
    ...
    if (curIndex > 0 && numChildren() > 0
        && this->getY() < 0 && getChildAt(0)->getY() > - this->getY())
        handleScroll1(false);
...
void GList::handleScroll2(bool forceUpdate)
{
    ...
    if (curIndex > 0 && numChildren() > 0 && this->getX() < 0
        && getChildAt(0)->getX() > -this->getX())
        handleScroll2(false);
...
void GList::handleAlign(float contentWidth, float contentHeight)
{
    ...
    if (!newOffset.equals(_alignOffset))
    {
        ...
        {
            this->setPosition2(_margin.left + _alignOffset.x, _margin.top + _alignOffset.y);
        }
    }
}
