...
void GSlider::updateWidthPercent(float percent)
{
    if (_titleObject != nullptr)
    {
        ...
        case ProgressTitleType::VALUE_MAX:
            oss += AZStd::to_string(round(_value)) + "/" + AZStd::to_string(round(_max));
            break;
...
void GSlider::constructFromXML(TXMLElement * xml)
{
    ...
    {
        _gripObject->addEventListener(UIEventType::TouchBegin, FUI_CALLBACK_1(GSlider::onGripTouchBegin, this));
        _gripObject->addEventListener(UIEventType::TouchMove, FUI_CALLBACK_1(GSlider::onGripTouchMove, this));
    }

    addEventListener(UIEventType::TouchBegin, FUI_CALLBACK_1(GSlider::onTouchBegin, this));
}
...
void GSlider::onTouchBegin(EventContext * context)
{
    ...
    if (evt->getButton() != MouseButton::BUTTON_LEFT)
        return;
