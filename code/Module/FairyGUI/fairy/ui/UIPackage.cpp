...
unordered_map<AZStd::string, UIPackage*> UIPackage::_packageInstById;
unordered_map<AZStd::string, UIPackage*> UIPackage::_packageInstByName;
list<UIPackage*> UIPackage::_packageList;
...
void UIPackage::removePackage(const string& packageIdOrName)
{
    ...
    if (pkg)
    {
        auto it = find(_packageList.begin(), _packageList.end(), pkg);
        if (it != _packageList.end())
        ...
        f_printf("FairyGUI: invalid package name or id: %s", packageIdOrName.c_str());
}
...
PixelHitTestData * UIPackage::getPixelHitTestData(const AZStd::string & itemId)
{
...
void UIPackage::create(const string& assetPath)
{
    ...
    if (FileUtils::getInstance()->loadAsset(assetPath + ".bytes", data) != FileUtils::Status::OK)
    {
        f_printf("FairyGUI: cannot load package from '%s'", assetPath.c_str());
        return;
    }

    if (_emptyTexture == nullptr)
    {
        _emptyTexture = Texture2D::createEmpty();
    }
...
void UIPackage::loadPackage()
{
    ...
    if (FileUtils::getInstance()->loadAsset(_assetNamePrefix + "sprites.bytes", str_all) != FileUtils::Status::OK)
    {
        f_printf("FairyGUI: cannot load package from '%s'", _assetNamePrefix.c_str());
        return;
    }
    ...
    if (FileUtils::getInstance()->isFileExist(hitTestDataFilePath))
    {
        Data data;
        FileUtils::getInstance()->loadAsset(hitTestDataFilePath, data);
    ...
    if (rxml == nullptr)
    {
        f_printf("FairyGUI: invalid package xml '%s'", _assetNamePrefix.c_str());
        return;
    }
...
//        f_printf("FairyGUI: %s not found in %s", sprite->atlas.c_str(), _name.c_str());
    }
...
void UIPackage::loadMovieClip(PackageItem * item)
{
    ...
    Size mcSize = Size(item->width, item->height);
        ...
        if (!spriteId.empty())
        {
            ...
            if (it != _sprites.end())
            {
                ...
                if (atlasItem)
                {
                    ...
                    spriteFrame->setOriginalSizeInPixels(mcSize);
...
//        spriteFrame->setOffset(rect.origin);
        spriteFrame->setRectInPixels(rect);
        AnimationFrame* frame = AnimationFrame::create(spriteFrame, addDelay / interval + 1);
...
//                tempRect = CC_RECT_PIXELS_TO_POINTS(tempRect);
            ...
            else if (charImg)
            {
                ...
                Rect tempRect = charImg->spriteFrame->_rect;
...
//                tempRect = CC_RECT_PIXELS_TO_POINTS(tempRect);
                ...
                if (mainTexture == nullptr)
                    mainTexture = charImg->spriteFrame->texture;
                def.validDefinition = true;
                def.texture = charImg->spriteFrame->texture;

                item->refSpriteFrame = charImg->spriteFrame;

                if (def.xAdvance == 0)
                {
                    if (xadvance == 0)
                    {
                        def.xAdvance = def.offsetX + def.width;
                    }
                    else
                    {
            ...
            item->bitmapFont->fontAtlas.insert(make_pair(charId, def));
...
GObject * UIPackage::createObject(const string & resName)
{
    ...
    AZStd::string ss = AZStd::string::format("FairyGUI: resource not found - %s in  %s",
        resName.c_str(), _name.c_str());

    f_assert2(pi, ss.c_str());
