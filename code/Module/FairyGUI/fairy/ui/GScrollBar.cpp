...
void GScrollBar::constructFromXML(TXMLElement * xml)
{
    ...
    _grip = getChild("grip");
    f_assert2(_grip != nullptr, "FairyGUI: should define grip");
    _bar = getChild("bar");
    f_assert2(_bar != nullptr, "FairyGUI: should define bar");

    _arrowButton1 = getChild("arrow1");
    _arrowButton2 = getChild("arrow2");

    _grip->addEventListener(UIEventType::TouchBegin, FUI_CALLBACK_1(GScrollBar::onGripTouchBegin, this));
    _grip->addEventListener(UIEventType::TouchMove, FUI_CALLBACK_1(GScrollBar::onGripTouchMove, this));
    this->addEventListener(UIEventType::TouchBegin, FUI_CALLBACK_1(GScrollBar::onTouchBegin, this));

    if (_arrowButton1 != nullptr)
        _arrowButton1->addEventListener(UIEventType::TouchBegin, FUI_CALLBACK_1(GScrollBar::onArrowButton1Click, this));
    if (_arrowButton2 != nullptr)
        _arrowButton2->addEventListener(UIEventType::TouchBegin, FUI_CALLBACK_1(GScrollBar::onArrowButton2Click, this));
}
