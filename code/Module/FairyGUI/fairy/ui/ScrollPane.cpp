...
static inline float sp_getField(const Size& sz, int axis) { return axis == 0 ? sz.width : sz.height; }
    ...
    _scrollY(0)
{
    ...
    _maskContainer = GComponent::create();
    _maskContainer->retain();
    _maskContainer->_parent = _owner;
    _maskContainer->setPosition(0, 0);
    _maskContainer->setClippingEnabled(true);

    this->setScrollPos(0, 0);

    _scrollBarMargin = scrollBarMargin;
    _scrollType = scrollType;
    _scrollStep = UIConfig::getInstance()->defaultScrollStep;
    _mouseWheelStep = _scrollStep * 2;
    _decelerationRate = UIConfig::getInstance()->defaultScrollDecelerationRate;
    ...
    if ((flags & 16) != 0)
        ...
        _touchEffect = UIConfig::getInstance()->defaultScrollTouchEffect;
    if ((flags & 64) != 0)
        _bouncebackEffect = true;
    else if ((flags & 128) != 0)
        _bouncebackEffect = false;
    else
        _bouncebackEffect = UIConfig::getInstance()->defaultScrollBounceEffect;
    ...
    if (scrollBarDisplay != ScrollBarDisplayType::HIDDEN)
    {
        ...
        {
            const AZStd::string& res = vtScrollBarRes.size() == 0 ? UIConfig::getInstance()->verticalScrollBar : vtScrollBarRes;
            if (res.length() > 0)
            {
                _vtScrollBar = UIPackage::createObjectFromURL(res)->SafeCast<GScrollBar>();
                if (_vtScrollBar == nullptr)
                    f_printf("FairyGUI: cannot create scrollbar from %s", res.c_str());
                else
                {
                    _vtScrollBar->retain();
        ...
        {
            const AZStd::string& res = hzScrollBarRes.length() == 0 ? UIConfig::getInstance()->horizontalScrollBar : hzScrollBarRes;
            if (res.length() > 0)
            {
                _hzScrollBar = UIPackage::createObjectFromURL(res)->SafeCast<GScrollBar>();
        ...
        if (_scrollBarDisplayAuto)
        {
            ...
            _owner->addEventListener(UIEventType::RollOver, FUI_CALLBACK_1(ScrollPane::onRollOver, this));
            _owner->addEventListener(UIEventType::RollOut, FUI_CALLBACK_1(ScrollPane::onRollOut, this));
        }
    }
    else
    ...
    {
        _header =  UIPackage::createObjectFromURL(headerRes)->SafeCast<GComponent>();
        if (_header == nullptr)
            f_printf("FairyGUI: cannot create scrollPane header from %s", headerRes.c_str());
        else
        {
            _header->retain();
    ...
    {
        _footer = UIPackage::createObjectFromURL(footerRes)->SafeCast<GComponent>();
        if (_footer == nullptr)
            f_printf("FairyGUI: cannot create scrollPane footer from %s", footerRes.c_str());
        else
        {
            _footer->retain();
    ...
    _owner->addEventListener(UIEventType::MouseWheel, FUI_CALLBACK_1(ScrollPane::onMouseWheel, this));
    _owner->addEventListener(UIEventType::TouchBegin, FUI_CALLBACK_1(ScrollPane::onTouchBegin, this));
    _owner->addEventListener(UIEventType::TouchMove, FUI_CALLBACK_1(ScrollPane::onTouchMove, this));
    _owner->addEventListener(UIEventType::TouchEnd, FUI_CALLBACK_1(ScrollPane::onTouchEnd, this));
}
...
ScrollPane::~ScrollPane()
{
    ...
    if(_maskContainer)
    {
        _maskContainer->_parent = nullptr;
        _maskContainer->release();
    }
}
...
void ScrollPane::scrollToView(GObject * obj, bool ani, bool setFirst)
{
    ...
    Rect rect = Rect(obj->getPosition().x, obj->getY(), obj->getWidth(), obj->getHeight());
    if (obj->getParent() != _owner)
        rect = obj->getParent()->transformRect(rect, _owner);
    scrollToView(rect, ani, setFirst);
}

void ScrollPane::scrollToView(const Rect & rect, bool ani, bool setFirst)
{
    _owner->ensureBoundsCorrect();
    if (_needRefresh)
        refresh();
...
bool ScrollPane::isChildInView(GObject * obj) const
{
    if (_overlapSize.height > 0)
    {
        float dist = obj->getY() + this->getScrollY();
        if (dist <= -obj->getHeight() || dist >= _viewSize.height)
            return false;
    }
    if (_overlapSize.width > 0)
    {
        float dist = obj->getPosition().x + this->getScrollX();
...
{
    return clampf(-this->getScrollX(), 0, _overlapSize.width);
}

float ScrollPane::getScrollingPosY() const
{
    return clampf(-this->getScrollY(), 0, _overlapSize.height);
}
...
void ScrollPane::adjustMaskContainer()
{
    ...
    // _maskContainer->setPosition(mx, _owner->getHeight() - _viewSize.height - my);
    _maskContainer->setPosition(mx, my, true, false);
}
...
void ScrollPane::changeContentSizeOnScrolling(float deltaWidth, float deltaHeight, float deltaPosX, float deltaPosY)
{
    ...
    else if (_tweening == 2)
    {
        if (deltaPosX != 0)
        {
            this->setScrollX(this->getScrollX() - deltaPosX);
            _tweenStart.x -= deltaPosX;
            _xPos = -this->getScrollX();
        }
        if (deltaPosY != 0)
        {
            this->setScrollY(this->getScrollY() - deltaPosY);
            _tweenStart.y -= deltaPosY;
            _yPos = -this->getScrollY();
        }
    }
    ...
    else
    {
        ...
        {
            _xPos = _overlapSize.width;
            this->setScrollX(this->getScrollX() - _xPos);
        }

        if (deltaHeight != 0 && isBottom)
        {
            _yPos = _overlapSize.height;
            this->setScrollY(this->getScrollY() - _yPos);
        }
    }
...
void ScrollPane::handleSizeChanged()
{
    ...
    _maskContainer->setSize(_viewSize);
    _maskContainer->setClippingRegion(Rect(Vec2::ZERO, _viewSize));
    ...
    if (_refreshBarAxis == 0)
        this->setPos(clampf(this->getScrollX(), -max, _headerLockedSize),
            clampf(this->getScrollY(), -_overlapSize.height, 0));
    else
        this->setPos(clampf(this->getScrollX(), -_overlapSize.width, 0),
            clampf(this->getScrollY(), -max, _headerLockedSize));
...
GObject* ScrollPane::hitTest(const Vec2 & pt)
{
    GObject *target = nullptr;
    if (_vtScrollBar)
    {
        target = _vtScrollBar->hitTest(pt);
        if (target)
            return target;
    }
    if (_hzScrollBar)
    {
        target = _hzScrollBar->hitTest(pt);
        if (target)
            return target;
    }
    if (_header && _header->getParent() != nullptr)
    {
        target = _header->hitTest(pt);
        if (target)
            return target;
    }
    if (_footer && _footer->getParent() != nullptr)
    {
        target = _footer->hitTest(pt);
        if (target)
            return target;
    }
...
void ScrollPane::refresh2()
{
    if (_aniFlag == 1 && !_isMouseMoved)
    {
        ...
        if (pos.x != this->getScrollX() || pos.y != this->getScrollY())
        {
            _tweening = 1;
            _tweenTime.setZero();
            _tweenDuration.set(TWEEN_TIME_GO, TWEEN_TIME_GO);
            _tweenStart = this->getScrollPos();
    ...
    else
    {
        ...
        this->setScrollPos((int)-_xPos, (int)-_yPos);
...
void ScrollPane::syncScrollBar(bool end)
{
    if (_vtScrollBar != nullptr)
    {
        _vtScrollBar->setScrollPerc(_overlapSize.height == 0 ? 0 : clampf(-this->getScrollY(), 0, _overlapSize.height) / _overlapSize.height);
        if (_scrollBarDisplayAuto)
            showScrollBar(!end);
    }
    if (_hzScrollBar != nullptr)
    {
        _hzScrollBar->setScrollPerc(_overlapSize.width == 0 ? 0 : clampf(-this->getScrollX(), 0, _overlapSize.width) / _overlapSize.width);
        if (_scrollBarDisplayAuto)
            showScrollBar(!end);
    }
}
...
void ScrollPane::loopCheckingTarget(Vec2 & endPos)
{
    if (_loop == 1)
...
void ScrollPane::loopCheckingTarget(Vec2 & endPos, int axis)
{
...
void ScrollPane::loopCheckingNewPos(float & value, int axis)
{
    ...
    {
        if (axis == 0)
            this->setScrollX(-(int)pos);
        else
            this->setScrollY(-(int)pos);
    }
}
...
float ScrollPane::alignByPage(float pos, int axis, bool inertialScrolling)
{
    ...
    else
    {
        ...
        float change = inertialScrolling ? (pos - sp_getField(_containerPos, axis)) : (pos - sp_getField(this->getScrollPos(), axis));
        float testPageSize = MIN(pageSize, contentSize - (page + 1) * pageSize);
        float delta = -pos - page * pageSize;

        if (abs(change) > pageSize)
        {
...
Vec2 ScrollPane::updateTargetAndDuration(const Vec2 & orignPos)
{
    Vec2 ret(0, 0);
...
#else
        const Size& winSize = Size(800, 600);// Director::getInstance()->getWinSizeInPixels();
...
void ScrollPane::fixDuration(int axis, float oldChange)
{
    ...
    float newDuration = abs(tweenChange / oldChange) * sp_getField(_tweenDuration, axis);
...
void ScrollPane::checkRefreshBar()
{
    ...
    float pos = sp_getField(this->getScrollPos(), _refreshBarAxis);
    if (_header != nullptr)
    {
        if (pos > 0)
        {
...
float ScrollPane::runTween(int axis, float dt)
{
    ...
    if (sp_getField(_tweenChange, axis) != 0)
    {
        ...
        newValue = sp_getField(this->getScrollPos(), axis);
...
void ScrollPane::onTouchBegin(EventContext * context)
{
    ...
    Vec2 pt = evt->getPosition();// _owner->globalToLocal(evt->getPosition());
//    pt.y = UIRoot->getHeight() - pt.y;
    ...
    _containerPos = this->getScrollPos();
...
//    pt.y = UIRoot->getHeight() - pt.y;
    ...
    sensitivity = 8;
#else
    sensitivity = UIConfig::getInstance()->touchScrollSensitivity;
#endif
    sensitivity = 0;
...
//    Vec2 newPos = _beginTouchPos - _containerPos - pt;
    ...
    if (sv)
    {
        if (newPos.y > 0)
        {
            if (!_bouncebackEffect)
                ...
                this->setScrollY(((int)MIN(newPos.y * 0.5f, _header->maxSize.height)));
            else
                this->setScrollY(((int)MIN(newPos.y * 0.5f, _viewSize.height * PULL_RATIO)));
        }
        else if (newPos.y < -_overlapSize.height)
        {
            if (!_bouncebackEffect)
                this->setScrollY(-_overlapSize.height);
            else if (_footer != nullptr && _footer->maxSize.height > 0)
                this->setScrollY(((int)MAX((newPos.y + _overlapSize.height) * 0.5f, -_footer->maxSize.height) - _overlapSize.height));
            else
                this->setScrollY(((int)MAX((newPos.y + _overlapSize.height) * 0.5f, -_viewSize.height * PULL_RATIO) - _overlapSize.height));
        }
        else
            this->setScrollY(newPos.y);
    }
    ...
    if (sh)
    {
        if (newPos.x > 0)
        {
            if (!_bouncebackEffect)
                ...
                this->setScrollX((int)MIN(newPos.x * 0.5f, _header->maxSize.width));
            else
                this->setScrollX((int)MIN(newPos.x * 0.5f, _viewSize.width * PULL_RATIO));
        }
        else if (newPos.x < 0 - _overlapSize.width)
        {
            if (!_bouncebackEffect)
                this->setScrollX(-_overlapSize.width);
            else if (_footer != nullptr && _footer->maxSize.width > 0)
                this->setScrollX((int)MAX((newPos.x + _overlapSize.width) * 0.5f, -_footer->maxSize.width) - _overlapSize.width);
            else
                this->setScrollX((int)MAX((newPos.x + _overlapSize.width) * 0.5f, -_viewSize.width * PULL_RATIO) - _overlapSize.width);
        }
        else
            this->setScrollX(newPos.x);
    }

    auto deltaTime = FGuiCentral::getInstance()->deltaTime;// 0.033f;// Director::getInstance()->getDeltaTime();
    ...
    if (_overlapSize.width > 0)
        _xPos = clampf(-this->getScrollX(), 0, _overlapSize.width);
    if (_overlapSize.height > 0)
        _yPos = clampf(-this->getScrollY(), 0, _overlapSize.height);

    if (_loop != 0)
    {
        newPos = this->getScrollPos();
        if (loopCheckingCurrent())
            _containerPos += this->getScrollPos() - newPos;
    }
...
void ScrollPane::onTouchEnd(EventContext * context)
{
    ...
    else if (this->getScrollX() < -_overlapSize.width)
    {
    ...
    else if (this->getScrollY() < -_overlapSize.height)
    {
    ...
    if (flag)
    {
        ...
        if (_tweenChange.x < -UIConfig::getInstance()->touchDragSensitivity || _tweenChange.y < -UIConfig::getInstance()->touchDragSensitivity)
            _owner->dispatchEvent(UIEventType::PullDownRelease);
        else if (_tweenChange.x > UIConfig::getInstance()->touchDragSensitivity || _tweenChange.y > UIConfig::getInstance()->touchDragSensitivity)
...
void ScrollPane::setScrollX(float value)
{
    _scrollX = value; 
}
void ScrollPane::setScrollY(float value)
{
    _scrollY = value; 
}
void ScrollPane::setScrollPos(float x, float y)
{
    _scrollX = x; 
    _scrollY = y; 
}
