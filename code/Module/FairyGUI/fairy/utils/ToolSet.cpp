...
void ToolSet::splitString(const string &s, char delim, vector<string> &elems)
{
    ...
    AZ::StringFunc::Tokenize(s.c_str(), elems, delim, true ,true);
}

void ToolSet::splitString(const string &s, char delim, Vec2& value, bool intType)
{
...
void ToolSet::splitString(const string &s, char delim, Vec4& value, bool intType)
{
    vector<string> helperArray;
    splitString(s, delim, helperArray);
    if (intType)
...
void ToolSet::splitString(const AZStd::string & s, char delim, AZStd::string & str1, AZStd::string& str2)
{
    vector<string> helperArray;
    splitString(s, delim, helperArray);
    str1 = helperArray[0];
...
int ToolSet::findInStringArray(const vector<AZStd::string>& arr, const AZStd::string& str)
{
    auto iter = find(arr.begin(), arr.end(), str);
    if (iter != arr.end())
...
TextVAlignment ToolSet::parseVerticalAlign(const char * p)
{
    if (!p)
...
FillMethod ToolSet::parseFillMethod(const char * p)
{
    if (!p)
    ...
    case "radial180"_hash:
        return FillMethod::RADIAL180;
    case "radial360"_hash:
...
TweenType ToolSet::parseEaseType(const char * p)
{
    if (!p)
        return Expo_EaseOut;
    ...
    case "Elastic.In"_hash:
        return Elastic_EaseIn;
    case "Elastic.Out"_hash:
        return Elastic_EaseOut;
    case "Elastic.InOut"_hash:
        return Elastic_EaseInOut;
    ...
    case "Quad.InOut"_hash:
        return Quad_EaseInOut;

    case "Cube.In"_hash:
        return Cubic_EaseIn;
    case "Cube.Out"_hash:
        return Cubic_EaseOut;
    case "Cube.InOut"_hash:
        return Cubic_EaseInOut;

    case "Quart.In"_hash:
        return Quart_EaseIn;
    case "Quart.Out"_hash:
        return Quart_EaseOut;
    case "Quart.InOut"_hash:
        return Quart_EaseInOut;
    ...
    case "Sine.InOut"_hash:
        return Sine_EaseInOut;

    case "Bounce.In"_hash:
        return Bounce_EaseIn;
    case "Bounce.Out"_hash:
        return Bounce_EaseOut;
    case "Bounce.InOut"_hash:
        return Bounce_EaseInOut;
